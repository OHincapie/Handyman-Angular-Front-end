...
export class CalculatorFormComponent implements OnInit {
  ...
  @Output() public hoursToEmit = new EventEmitter<CalculatorHoursResponse[]>();
  @Output() public totalHoursToEmit = new EventEmitter<CalculatorHoursResponse>();
  ...
  consultHours(serviceTechnician: ServiceTechnicianModel) {
    ...
    this.calculatorService.consultHoursWorked(serviceTechnician).subscribe({
      next: (hours: CalculatorHoursResponse[]) => {
        this.hoursToEmit.emit(hours);
        if (hours.length === 0) {
          this.notifierService.notify('success', 'No hay registros del técnico en esas fechas.');
        } else {
          this.totalHoursToEmit.emit(this.calculateTotalHours(hours));
  ...
  onClickWeek(event: any) {
    this.initialDateAux = new Date("2022/01/03 0:0:0");
    this.finalDateAux = new Date("2022/01/09 23:59:59");
    const initialDate = new Date("2022/01/03 0:0:0");
  ...
  clean() {
    this.resetFormGroup();
    this.hoursToEmit.emit(undefined);
  }

  calculateTotalHours(hours: CalculatorHoursResponse[]): CalculatorHoursResponse {
    const totalHours: CalculatorHoursResponse = {
      regularHours : 0,
      nightHour: 0,
      sundayHour: 0,
      regularHourExtra: 0,
    ...
    totalHours.regularHours = hours.map(i => i.regularHours).reduce((prev, curr) => prev + curr, 0);
    totalHours.nightHour = hours.map(i => i.nightHour).reduce((prev, curr) => prev + curr, 0);
    totalHours.sundayHour = hours.map(i => i.sundayHour).reduce((prev, curr) => prev + curr, 0);
    totalHours.regularHourExtra = hours.map(i => i.regularHourExtra).reduce((prev, curr) => prev + curr, 0);
    totalHours.nightHourExtra = hours.map(i => i.nightHourExtra).reduce((prev, curr) => prev + curr, 0);
    totalHours.sundayHourExtra = hours.map(i => i.sundayHourExtra).reduce((prev, curr) => prev + curr, 0);
    totalHours.totalHours = hours[hours.length - 1].totalHours;
    return totalHours;
